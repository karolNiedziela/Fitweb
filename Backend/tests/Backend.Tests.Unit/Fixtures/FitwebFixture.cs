...
namespace Backend.Tests.Unit.Fixtures
{
    public class FitwebFixture : IDisposable
    {
        ...
        {
            var options = new DbContextOptionsBuilder<FitwebContext>()
                .UseInMemoryDatabase(databaseName: $"UnitTestDb{Guid.NewGuid()}")
                .Options;

            FitwebContext = new FitwebContext(options);
